...
object ThumbHash {
    ...
    fun rgbaToThumbHash(w: Int, h: Int, rgba: ByteArray): ByteArray {
        ...
        val lx = max(1, Math.round((l_limit * w).toFloat() / max(w, h).toFloat()))
        val ly = max(1, Math.round((l_limit * h).toFloat() / max(w, h).toFloat()))
        ...
        val l_channel = Channel(max(3, lx), max(3, ly)).encode(w, h, l)
    ...
    fun thumbHashToRGBA(hash: ByteArray): Image {
        ...
        while (y < h) {
            ...
            while (x < w) {
                ...
                for (cx in 0 until cx_stop) fx[cx] = cos(Math.PI / w * (x + 0.5f) * cx).toFloat()
                for (cy in 0 until cy_stop) fy[cy] = cos(Math.PI / h * (y + 0.5f) * cy).toFloat()
                ...
                val g = r - q
                rgba[i] = max(0, Math.round(255.0f * min(1f, r))).toByte()
                rgba[i + 1] = max(0, Math.round(255.0f * min(1f, g))).toByte()
                rgba[i + 2] = max(0, Math.round(255.0f * min(1f, b))).toByte()
                rgba[i + 3] = max(0, Math.round(255.0f * min(1f, a))).toByte()
                x++
                i += 4
            }
            y++
    ...
    fun thumbHashToAverageRGBA(hash: ByteArray): RGBA {
        ...
        val r = (3.0f * l - b + q) / 2.0f
        val g = r - q
        return RGBA(max(0f, min(1f, r)).toFloat(), max(0f, min(1f, g)).toFloat(), max(0f, min(1f, b)).toFloat(),
                a)
    }
